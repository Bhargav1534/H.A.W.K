...
import faiss
from sentence_transformers import SentenceTransformer
import memory.knowledge as tk

embedder = SentenceTransformer("all-MiniLM-L6-v2")
embeddings = embedder.encode(tk.tools, convert_to_numpy=True)
dimension = embeddings.shape[1]
tools_index = faiss.IndexFlatL2(dimension)
tools_index.add(embeddings)

def retrieve_tools(data, k=3) -> list[str]:
    enc = embedder.encode([data], convert_to_numpy=True)
    D, I = tools_index.search(enc, k)
    return [tk.tools[i] for i in I[0] if i < len(tk.tools)]

print("Tool Retrieval Test:", retrieve_tools("clear everything for now"))